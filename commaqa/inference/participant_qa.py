...
class LLMQAParticipantModel(ParticipantModel):
    ...
    def query(self, state, debug=False):
        ...
        if self.store_sents_in_generated_sentences:
            ...
            state.data["generated_sentences"] = []
            for answer in answer_list:
                state.data["generated_sentences"].append(
                    [sent.text_with_ws for sent in spacy_object(answer).sents]
                )

        # return self.update_state(answer=answer, state=state)
        return self.update_state(answer=json.dumps(answer_list), state=state)
