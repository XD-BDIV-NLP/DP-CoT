...
from discriminator.dis_model import get_dis_model_and_tokenizer
import torch
import numpy as np
...
class AnswerExtractor(ParticipantModel):
    ...
    def query(self, state, debug=False):
        ...
        # while type(query) is str and query[0] == "[" and query[-1] == ']':
        #     query = json.loads(query)
        while type(query) is str and "[" in query[:3] and ']' in query[-3:]:
            query = json.loads(query)

        if type(query) is list:
            for i in range(len(query)):
                if query[i].startswith('"') and query[i].endswith('"'):
                    query[i] = query[i][1:-1]
        else:
        ...
        if type(query) is list:
            ...
            for query_elem in query:
                m = self.regex.match(query_elem)

                if self.match_all_on_failure and not self.regex.match(query_elem):
                    m = re.compile(r"(.*)").match(query_elem)
            ...
            new_state.data.add_answer(QuestionAnsweringStep(answer=json.dumps(answer_list), score=0, participant=state.next))
            new_state.last_output = answer_list
...
class RetrieveAndResetParagraphsParticipant(ParticipantModel):
    ...
    def query(self, state, debug=False):
        ...
        if self.query_source == "original_question":
            ...
            input_query_list.append(input_query)

        elif self.query_source == "last_answer":
            # input_query = state.data.get_last_answer()
            input_query_list = json.loads(state.data.get_last_answer())
            ...
            generated_sentences_list = state.data.get("generated_sentences", [])
            # print("***" * 20)
            # print("generated_sentences_list is " + str(generated_sentences_list))
            # print("***" * 20)
            if not generated_sentences_list:
                input_query_list.append(question)
            else:
                for generated_sentences in generated_sentences_list:
        ...
        # print("***" * 20)
        # print("the length of input_query_list is " + str(len(input_query_list)))
        # print("***" * 20)

        # if self.cumulate_titles:
        #     selected_titles = state.data["titles"]
        #     selected_paras = state.data["paras"]
        if self.cumulate_titles and state.data["titles"]:
            selected_titles = state.data["titles"]
            selected_paras = state.data["paras"]
        else:
            # selected_titles = []
        ...
        if self.retrieval_type == "map_generated_to_valid_titles":
            ...
            for idx, input_query in enumerate(input_query_list):
                ...
                for generated_title in generated_titles:
                    ...
                    if result.ok:
                        ...
                        for retrieval_ in retrieval:
                            ...
                            if selected_title not in selected_titles[idx] and len(selected_paras[idx]) < self.global_max_num_paras:
                                # 根据Q检索(input_query_list中只有一个问题，长度为1)，检索结果应放入两条检索链中
                                if len(input_query_list) < 2:
                                    for i in range(len(selected_titles)):
                                        selected_titles[i].append(selected_title)
                                        selected_paras[i].append(selected_para)
                                # 根据两条rationales检索，检索结果分别放入两条检索链
                                else:
                                    selected_titles[idx].append(selected_title)
                                    selected_paras[idx].append(selected_para)
            ...
            for idx, input_query in enumerate(input_query_list):
                ...
                for allowed_paragraph_type in self.allowed_paragraph_types:
                    ...
                    if self.valid_titles_are_allowed_titles:
                        params["allowed_titles"] = state.data["valid_titles"][idx]
                    ...
                    if result.ok:
                        ...
                        for retrieval_item in retrieval:
                            ...
                            # print("***" * 20)
                            # print("selected_titles is " + str(selected_titles))
                            # print("self.cumulate_titles is " + str(self.cumulate_titles))
                            # print("***" * 20)
                            ...
                            if len(selected_paras[idx]) >= self.global_max_num_paras:
                                continue

                            if self.valid_titles_are_allowed_titles:
                                assert retrieval_item["title"].lower().replace(" ", "") in [
                                    valid_title.lower().replace(" ", "") for valid_title in state.data["valid_titles"][idx]
                                ]

                            # 根据Q检索(input_query_list中只有一个问题，长度为1)，检索结果应放入两条检索链中
                            if len(input_query_list) < 2:
                                for i in range(len(selected_titles)):
                                    selected_titles[i].append(retrieval_item["title"])
                                    selected_paras[i].append(retrieval_item["paragraph_text"])
                            # 根据两条rationales检索，检索结果分别放入两条检索链
                            else:
                                selected_titles[idx].append(retrieval_item["title"])
                                selected_paras[idx].append(retrieval_item["paragraph_text"])
        ...
        if self.return_pids:
            ...
            for i in range(len(selected_titles)):
                pids.append([
                    get_pid_for_title_paragraph_text(title, paragraph_text)
                    for title, paragraph_text in zip(selected_titles[i], selected_paras[i])
                ])
        ...
        if self.return_paras:
            ...
            for i in range(len(selected_titles)):
                paras.append([
                    {"title": title, "paragraph_text": para} for title, para in zip(selected_titles[i], selected_paras[i])
                ])
            answer = json.dumps(paras)
...
class StepByStepLLMTitleGenParticipant(ParticipantModel):
    ...
    def query(self, state, debug=False):
        ...
        # generated_titles_str_list = []
        # for output_text_scores_elem in output_text_scores:
        #     generated_titles_str_list.append(output_text_scores_elem[0].strip())

        new_state = state.copy()
        new_state.data.add_answer(QuestionAnsweringStep(answer=generated_titles_str, score=0, participant=state.next))
        # new_state.data.add_answer(QuestionAnsweringStep(answer=generated_titles_str_list, score=0, participant=state.next))
        new_state.next = self.next_model if self.next_model else self.end_state
...
class StepByStepCOTGenParticipant(ParticipantModel):
    ...
    def query(self, state, debug=False):
        ...
        for elem in state.data[f"generated_{self.generation_type}"]:
            if len(elem) >= self.max_num_sentences:
                exit_generation = True

        new_state = state.copy()
        return_answer = "EMPTY"
        return_answer_list = [return_answer, return_answer]  # return_answer is "EMPTY"
        # return_titles = json.dumps(state.data["titles"])
        return_titles_list = json.dumps(state.data["titles"])
        ...
        # )
        pids_list = []
        for i in range(len(state.data["titles"])):
            pids_list.append(
                [
                    get_pid_for_title_paragraph_text(title, paragraph_text)
                    for title, paragraph_text in zip(state.data["titles"][i], state.data["paras"][i])
                ]
            )
        return_pids_list = json.dumps(pids_list)
        ...
        if not exit_generation:
            ...
            for i in range(len(state.data["titles"])):
                ...
                if not state.data[f"generated_{self.generation_type}"]:
                    generation_so_far = ""
                else:
                    generation_so_far = " ".join(state.data[f"generated_{self.generation_type}"][i])

                # print("***" * 20)
                # print(state.data[f"generated_{self.generation_type}"])
                # print("***" * 20)
                # print(generation_so_far)
                # print("***" * 20)

                # if len(state.data[f"generated_{self.generation_type}"]) > 1:
                #     with open("./generation_so_far_more_than_one.txt", 'a', encoding='utf-8') as fg:
                #         fg.write(str(state.data[f"generated_{self.generation_type}"]) + '\n')
                #     fg.close()

                # with open("./question.txt", "a", encoding='utf-8') as fq:
                #     fq.writelines("***" * 20 + '\n')
                #     fq.write(question + '\n')
                #     fq.writelines("***" * 20 + '\n')
                # fq.close()
                # with open("./generation_so_far.txt", 'a', encoding='utf-8') as fg:
                #     fg.writelines("***" * 20 + '\n')
                #     fg.write(generation_so_far + '\n')
                #     fg.writelines("***" * 20 + '\n')
                # fg.close()
                ...
                # with open("./prompt.txt", "a", encoding='utf-8') as fp:
                #     fp.writelines("***" * 200 + '\n')
                #     fp.write(prompt + '\n')
                #     fp.writelines("***" * 200 + '\n')
                # fp.close()
                ...
                # output_text_scores = self.generator.generate_text_sequence(prompt)
                output_text_scores_elem = self.generator.generate_text_sequence(prompt)
                output_text_scores.append(output_text_scores_elem)
            # if len(output_text_scores) > 1:
            #     print("Can not handle more than one answer for this model yet" + "\n" + str(output_text_scores))

            # with open("./output_text_scores.txt", 'a', encoding='utf-8') as fn:
            #     fn.writelines("***" * 20 + '\n')
            #     fn.write(str(output_text_scores) + '\n')
            #     fn.writelines("***" * 20 + '\n')
            # fn.close()

            # print()
            # print("***" * 20)
            # print("output_text_scores: " + str(output_text_scores))
            # print("***" * 20)
            # time.sleep(2)
            # # print(type(output_text_scores[0][0]))  # str
            # # print("***" * 20)
            # print()

            new_generation = output_text_scores[0][0][0].strip()
            new_sents = list(self.spacy_object(new_generation).sents)

            if new_sents:

                dis_tokenizer, dis_model, dis_model_max_len = get_dis_model_and_tokenizer()
                input_for_dis_list = []
                d = {}
                d_count = 0

                for idx, output_text_scores_elem in enumerate(output_text_scores):
                    # 判别器数据构成：拼接所有选取段落的第一句话(paragraph level)
                    for i in range(len(output_text_scores_elem)):
                        para = output_text_scores_elem[i][0].strip()

                        para_sents = list(self.spacy_object(para).sents)
                        if not state.data[f"generated_{self.generation_type}"]:
                            input_for_dis = "[CLS] " + state.data["question"].strip() + \
                                            (" [SEP] " + para_sents[0].text) if para_sents else ""
                        else:
                            input_for_dis = "[CLS] " + state.data["question"].strip() + \
                                            (" [SEP] " if state.data[f"generated_{self.generation_type}"] != [] else "") + \
                                            " [SEP] ".join(state.data[f"generated_{self.generation_type}"][idx]) + \
                                            (" [SEP] " + para_sents[0].text) if para_sents else ""
                        input_for_dis_list.append(input_for_dis)
                        d[d_count] = para_sents[0].text if para_sents else ""
                        d_count += 1

                    # # 判别器数据构成：拼接第一个段落的每一句话(sentence level)
                    # para = output_text_scores_elem[0][0].strip()
                    # para_sents = list(self.spacy_object(para).sents)
                    # for i in range(len(para_sents)):
                    #     if not state.data[f"generated_{self.generation_type}"]:
                    #         input_for_dis = "[CLS] " + state.data["question"].strip() + \
                    #                         (" [SEP] " + para_sents[i].text) if para_sents else ""
                    #     else:
                    #         input_for_dis = "[CLS] " + state.data["question"].strip() + \
                    #                         (" [SEP] " if state.data[f"generated_{self.generation_type}"] != [] else "") + \
                    #                         " [SEP] ".join(state.data[f"generated_{self.generation_type}"][idx]) + \
                    #                         (" [SEP] " + para_sents[i].text) if para_sents else ""
                    #     input_for_dis_list.append(input_for_dis)
                    #     d[d_count] = para_sents[i].text if para_sents else ""
                    #     d_count += 1

                        # print()
                        # print("***" * 20)
                        # print("input_for_dis is " + str(input_for_dis))
                        # print("***" * 20)
                        # time.sleep(2)

                # for i in range(len(output_text_scores)):
                #     para = output_text_scores[i][0].strip()
                #
                #     # 判别器数据构成：拼接选取段落的第一句话
                #     para_sents = list(self.spacy_object(para).sents)
                #     input_for_dis = "[CLS] " + state.data["question"].strip() + (
                #         " [SEP] " if state.data[f"generated_{self.generation_type}"] != [] else "") + \
                #                     " [SEP] ".join(state.data[f"generated_{self.generation_type}"]) + " [SEP] " + \
                #                     para_sents[0].text
                #     input_for_dis_list.append(input_for_dis)
                #     d[i] = para_sents[0].text
                #     # d[i] = para
                #
                #     # 判别器数据构成：拼接整个段落
                #     input_for_dis = "[CLS] " + state.data["question"].strip() + (
                ...
                dis_inputs = dis_tokenizer(text=input_for_dis_list,
                                           return_tensors='pt',
                                           max_length=dis_model_max_len,
                                           padding="max_length",
                                           truncation=True)
                dis_outputs = dis_model(**dis_inputs)
                dis_predictions = torch.nn.functional.softmax(dis_outputs, dim=-1)
                # new_generation_idx = torch.argmax(dis_predictions, dim=0).numpy().tolist()[1]
                new_generation_idx_list = (-dis_predictions[:, 1].detach().numpy()).argsort().tolist()[:2]
                # new_generation_idx_list = random.sample(list(d.keys()), 2)

                # new_generation = d[new_generation_idx]
                # # new_generation = list(self.spacy_object(d[new_generation_idx]).sents)[0].text
                new_generation_list = []  # new_generation_list[0]是最高分候选, new_generation_list[1]次之
                for idx in new_generation_idx_list:
                    new_generation_list.append(d[idx])

                # new_generation = new_sents[0].text
                # print("***" * 20)
                # print("new_generation_list is " + str(new_generation_list))
                # print("***" * 20)
                # print("the type of new generation is " + str(type(new_generation)))  # str
                # print("***" * 20)
                # time.sleep(2)
                ...
                '''
                    CoT拼接
                '''
                if not new_state.data[f"generated_{self.generation_type}"]:  # new_state.data[f"generated_{self.generation_type}"] is []
                    for i in range(2):
                        new_state.data[f"generated_{self.generation_type}"].append([new_generation_list[i]])
                else:
                    pre_cot = new_state.data[f"generated_{self.generation_type}"]
                    for i, new_generation_idx in enumerate(new_generation_idx_list):
                        # # paragraph level
                        # pre_cot[i] = new_state.data[f"generated_{self.generation_type}"][new_generation_idx // len(output_text_scores[0])]
                        # sentence level
                        if new_generation_idx < len(list(self.spacy_object(output_text_scores[0][0][0].strip()).sents)):
                            pre_cot[i] = new_state.data[f"generated_{self.generation_type}"][0]
                        else:
                            pre_cot[i] = new_state.data[f"generated_{self.generation_type}"][1]
                        pre_cot[i].append(new_generation_list[i])
                    new_state.data[f"generated_{self.generation_type}"] = pre_cot
                    # pre = new_state.data[f"generated_{self.generation_type}"][0]
                    # for i in range(2):
                    #     pre.append(new_generation_list[i])
                    #     new_state.data[f"generated_{self.generation_type}"][i] = pre
                    #     pre = pre[:-1]

                for i, new_generation in enumerate(new_generation_list):
                    if self.answer_extractor_regex.match(new_generation):
                        tmp_return_answer = self.answer_extractor_regex.match(new_generation).group(1)
                        if self.answer_extractor_remove_last_fullstop and tmp_return_answer.endswith("."):
                            tmp_return_answer = tmp_return_answer[:-1]
                        return_answer_list[i] = tmp_return_answer
                        exit_generation = True
                ...
                if self.disable_exit:  # Add just empty sentence so exit controller can exit.
                    ...
                    for i in range(len(new_state.data[f"generated_{self.generation_type}"])):
                        new_state.data[f"generated_{self.generation_type}"][i].append("")
                exit_generation = True
        ...
        if exit_generation:
            ...
            output = [" ".join(elem) for elem in new_state.data[f"generated_{self.generation_type}"]]
            new_state.next = self.next_model
        ...
        new_state.data.add_answer(QuestionAnsweringStep(answer=json.dumps(output), score=0, participant=state.next))

        self.num_calls += 1

        # print()
        # print('***' * 20)
        # print("state.data: " + str(state.data))
        # print("new_state.data: " + str(new_state.data))
        # print('***' * 20)
        # print()
        # time.sleep(2)
...
class StepByStepExitControllerParticipant(ParticipantModel):
    ...
    def query(self, state, debug=False):
        ...
        generated_sentences_list = state.data[self.generation_key]

        new_state = state.copy()
        return_answer = "EMPTY"
        return_answer_list = ["EMPTY", "EMPTY"]
        # return_titles = json.dumps(state.data["titles"])
        return_titles_list = json.dumps(state.data["titles"])
        # return_pids = json.dumps(
        ...
        for i in range(len(state.data["titles"])):
            assert_unique_titles_paras(state.data["titles"][i], state.data["paras"][i])
            assert len(state.data["paras"][i]) <= self.global_max_num_paras

        exit_generation = False

        for i in range(len(state.data[self.generation_key])):
            if state.data[self.generation_key][i] and not state.data[self.generation_key][i][-1]:
                exit_generation = True

            if len(state.data[self.generation_key][i]) >= self.max_num_sentences:
                exit_generation = True
        ...
        for i, generated_sentences in enumerate(generated_sentences_list):
            if self.generation_key in ("generated_sub_answers", "generated_sub_questions"):
                tmp_return_answer = generated_sentences[-1]
            else:
                tmp_return_answer = " ".join(generated_sentences)
            return_answer_list[i] = tmp_return_answer

        for i, generated_sentences in enumerate(generated_sentences_list):
            if generated_sentences and self.answer_extractor_regex.match(generated_sentences[-1]):
                tmp_return_answer = self.answer_extractor_regex.match(generated_sentences[-1]).group(1)
                if self.answer_extractor_remove_last_fullstop and tmp_return_answer.endswith("."):
                    tmp_return_answer = tmp_return_answer[:-1]
                return_answer_list[i] = tmp_return_answer
                exit_generation = True
