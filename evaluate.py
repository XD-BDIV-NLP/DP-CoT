...
def evaluate_by_dicts(
    ...
    if prediction_type == "answer":
        metrics = [[DropAnswerEmAndF1(), SupportEmF1Metric(do_normalize_answer=True)],
                   [DropAnswerEmAndF1(), SupportEmF1Metric(do_normalize_answer=True)]]
    elif prediction_type in ("titles", "pids", "real_pids"):
        metrics = [[SupportEmF1Metric()], [SupportEmF1Metric()]]
    elif prediction_type in ("paras"):
        metrics = [[AnswerSupportRecallMetric()], [AnswerSupportRecallMetric()]]

    # print("***" * 20)
    # print("id_to_ground_truths is " + str(id_to_ground_truths))
    # print("***" * 20)
    # print("id_to_predictions is " + str(id_to_predictions))
    # print("***" * 20)
    ...
    for id_ in set(id_to_ground_truths.keys()):
        ...
        # print('***' * 20)
        # print("ground_truth is " + str(ground_truth))
        # print("type(prediction) is " + str(type(prediction)))
        # print("prediction is " + str(prediction))
        # print('***' * 20)

        for i, prediction_elem in enumerate(prediction):
            assert isinstance(prediction_elem, (str, list))
            if prediction_type == "answer" and isinstance(prediction_elem, str):
                if prediction_elem.strip().startswith("[") or prediction_elem.strip().endswith("]"):
                    prediction_elem = [e for e in prediction_elem.replace('"', "").replace("[", "").replace("]", "").split(",")]
                else:
                    prediction_elem = [prediction_elem]

            assert isinstance(prediction_elem, (list, tuple))
            prediction_elem = [str(e) for e in prediction_elem]

            if prediction_type == "answer":
                prediction_elem = [answer_extractor(_prediction) for _prediction in prediction_elem]  # Temporary.
                metrics[i][0](prediction_elem, [ground_truth])
                metrics[i][1](prediction_elem, ground_truth)
            elif prediction_type in ("titles", "pids", "real_pids"):
                metrics[i][0](prediction_elem, ground_truth)
            elif prediction_type in ("paras"):
                predicted_paras = [
                    " ".join([eval(prediction_)["title"], eval(prediction_)["paragraph_text"]])
                    for prediction_ in prediction_elem
                ]
                metrics[i][0](predicted_paras, ground_truth)

    evaluation_results = {}
    if prediction_type == "answer":
        evaluation_results = \
            metrics[0][0].get_metric() \
            if metrics[0][0].get_metric()["f1"] >= metrics[1][0].get_metric()["f1"] \
            else metrics[1][0].get_metric()
        evaluation_results_ = \
            metrics[0][1].get_metric() \
            if metrics[0][1].get_metric()["title_recall"] >= metrics[1][1].get_metric()["title_recall"] \
            else metrics[1][1].get_metric()
        ...
        evaluation_results = \
            metrics[0][0].get_metric() \
            if metrics[0][0].get_metric()["title_recall"] >= metrics[1][0].get_metric()["title_recall"] \
            else metrics[1][0].get_metric()
    elif prediction_type in ("paras"):
        evaluation_results = \
            metrics[0][0].get_metric() \
            if metrics[0][0].get_metric()["avg_predicted_paras"] >= metrics[1][0].get_metric()["avg_predicted_paras"] \
            else metrics[1][0].get_metric()
...
def official_evaluate_by_dicts(
    ...
    for id_, prediction in id_to_predictions.items():
        if type(prediction) is str:
            prediction = json.loads(prediction)
        id_to_predictions[id_] = []
        for i, prediction_elem in enumerate(prediction):
            # print(prediction_elem)
            # print(type(prediction_elem))
            if isinstance(prediction_elem, list) and len(prediction_elem) == 1:
                id_to_predictions[id_].append(str(prediction_elem[0]))
            elif isinstance(prediction_elem, list) and len(prediction_elem) > 1:
                id_to_predictions[id_].append(" ".join([str(e) for e in prediction]))
                print("WARNING: Found a list answer prediction, concatenating it.")
            elif isinstance(prediction_elem, str):
                id_to_predictions[id_].append(prediction_elem)
    ...
    if dataset == "hotpotqa":
        ...
        #     for prediction_elem in prediction:
        #         if not isinstance(prediction, str):
        #             print("WARNING: Found an answer prediction that's not a string.")

        data = {
            "answer": {id_: prediction for id_, prediction in id_to_predictions.items()},
    ...
    if dataset == "2wikimultihopqa":
        ...
        with open(temp_output_file_path, "r") as file:
            # print("***" * 20)
            # print("file.read().strip() is " + str(file.read().strip()))
            # print("***" * 20)
        ...
        # print("***" * 20)
        # print(temp_ground_truth_file_path)
        # print(temp_prediction_file_path)
...
def main():
    ...
    parser.add_argument("experiment_name_or_path", type=str, help="experiment_name_or_path",
                        default="instantiated_configs/ircot_qa_flan_t5_large_2wikimultihopqa____prompt_set_1___bm25_retrieval_count__2___distractor_count__1.jsonnet")
    parser.add_argument("evaluation_path", type=str, help="evaluation_path",
                        default="processed_data/2wikimultihopqa/dev_subsampled.jsonl")
